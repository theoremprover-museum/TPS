...
(load (concatenate 'string (concatenate 'string cl-user::tps-dir "bin\\boot0.")
		   compiled-extension) :verbose t)
(load (concatenate 'string (concatenate 'string cl-user::tps-dir "bin\\boot1.")
		   compiled-extension) :verbose t)
(load (concatenate 'string (concatenate 'string cl-user::tps-dir "bin\\defpck.")
		   compiled-extension) :verbose t)
...
(load-modules '( wffs wff-print wff-parse   
...
(load-modules '(mode-ml))

(in-mode ml (load-modules '(math-logic-2-wffs)))

(in-mode rules
  (in-mode ml (load-modules '(math-logic-2-rules ;;ml-tactics
))))
(in-mode ml (load-modules '(math-logic-2-exercises)))


(load-modules '(events etps-events)) ;; report 

; REMOVE XWINDOWS IF YOU CAN'T RUN X WINDOWS OR OTHER INTERFACE
;(load-modules '(xwindows library))
(load-modules '(xwindows))
(load-modules '(grader-top grader))
